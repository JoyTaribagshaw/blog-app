...
RSpec.describe 'User Show Page', type: :system do
  before :each do
    @user = User.create(name: 'User', photo: 'user.jpg', bio: 'User Bio')
    @user.posts.create(title: 'Post1', text: 'Content1')
    @user.posts.create(title: 'Post2', text: 'Content2')
    @user.posts.create(title: 'Post3', text: 'Content3')
  end

  it 'displays user data' do
    visit user_path(@user)
    expect(page).to have_content('User')
    expect(page).to have_selector("img[src*='user.jpg']")
    expect(page).to have_content('Number of posts: 3') # Adjust this according to your data
  ...
  it 'redirects to user post show page on post click' do
    visit user_path(@user)
    click_link('Post1') # Adjust the link text according to your implementation
    expect(page).to have_current_path(user_post_path(@user, @user.posts.first))
  end

  it 'redirects to user post index page on "See all posts" click' do
    visit user_path(@user)
    click_link('See all posts')
